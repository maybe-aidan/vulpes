...
b8 vulkan_swapchain_acquire_next_image_index(vulkan_context* context, vulkan_swapchain* swapchain, u64 timeout_ns, VkSemaphore image_available_semaphore, VkFence fence, u32* out_image_index){
    ...
    if(result == VK_ERROR_OUT_OF_DATE_KHR){
        // Trigger swapchain recreation, then boot out of render loop.
...
void vulkan_swapchain_present(vulkan_context* context, vulkan_swapchain* swapchain, VkQueue graphics_queue, VkQueue present_queue, VkSemaphore render_complete_semaphore, u32 present_image_index){
    // Return the image to the swapchain fro presentation
...
void create(vulkan_context* context, u32 width, u32 height, vulkan_swapchain* swapchain){
    ...
    if (!vulkan_device_detect_depth_format(&context->device)) {
        ...
        VFATAL("Failed to find a supported depth format!");
    }

    // Create depth image and its views
