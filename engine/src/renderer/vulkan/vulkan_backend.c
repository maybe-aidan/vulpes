...
b8 vulkan_renderer_backend_initialize(renderer_backend* backend, const char* application_name, struct platform_state* plat_state){
    ...
    app_info.pEngineName = "Vulpes Engine";
    ...
    platform_get_required_extension_names(&required_extensions);       // Platofrm specific extensions
    #if defined(_DEBUG)
    ...
    // If validation should be done, get a list of the required validatio layer names
    // and make sure they exist. Validation layers should only be enabled on non-release builds.
        ...
        // Verify availability of all layers
    ...
    if(!vulkan_device_create(&context)){
        VERROR("Failed to create Vulkan device!");
        return FALSE;
    }
